...
// TODO: Do we need this file? If not delete it
...
export class RunOnDevice {
    ...
    public static cleanup(): void {
        if (RunOnDevice.nativeDebuggerProxyInstance) {
            RunOnDevice.nativeDebuggerProxyInstance.kill('SIGHUP');
            RunOnDevice.nativeDebuggerProxyInstance = null;
        }
        if (RunOnDevice.webDebuggerProxyInstance) {
            RunOnDevice.webDebuggerProxyInstance.kill();
            RunOnDevice.webDebuggerProxyInstance = null;
        }
    }
    ...
    public static startDebugProxy(proxyPort: number): Q.Promise<child_process.ChildProcess> {
        if (RunOnDevice.nativeDebuggerProxyInstance) {
            RunOnDevice.nativeDebuggerProxyInstance.kill('SIGHUP'); // idevicedebugserver does not exit from SIGTERM
            RunOnDevice.nativeDebuggerProxyInstance = null;
        }

        return RunOnDevice.mountDeveloperImage().then(function (): Q.Promise<child_process.ChildProcess> {
            let deferred = Q.defer<child_process.ChildProcess>();
            RunOnDevice.nativeDebuggerProxyInstance = child_process.spawn('idevicedebugserverproxy', [proxyPort.toString()]);
            RunOnDevice.nativeDebuggerProxyInstance.on('error', function (err: any): void {
                deferred.reject(err);
            });
            // Allow 200ms for the spawn to error out, ~125ms isn't uncommon for some failures
            Q.delay(200).then(() => deferred.resolve(RunOnDevice.nativeDebuggerProxyInstance));
    ...
    public static startWebkitDebugProxy(proxyPort: number, proxyRangeStart: number, proxyRangeEnd: number): Q.Promise<any> {
        ...
        RunOnDevice.webDebuggerProxyInstance = child_process.spawn('ios_webkit_debug_proxy', ['-c', portRange]);
        RunOnDevice.webDebuggerProxyInstance.on('error', function (err: Error) {
    ...
    public static startApp(packageId: string, proxyPort: number, appLaunchStepTimeout: number): Q.Promise<string> {
        ...
        return RunOnDevice.getPathOnDevice(packageId).then(function (path: string): Q.Promise<string> { return RunOnDevice.startAppViaDebugger(proxyPort, path, appLaunchStepTimeout); });
    }
    ...
    public static startAppViaDebugger(portNumber: number, packagePath: string, appLaunchStepTimeout: number): Q.Promise<string> {
        let encodedPath: string = RunOnDevice.encodePath(packagePath);
        ...
        socket.connect(portNumber, 'localhost', function (): void {
            ...
            let cmd: string = RunOnDevice.makeGdbCommand('A' + encodedPath.length + ',0,' + encodedPath);
            initState++;
            socket.write(cmd);
        ...
        return deferred1.promise.then(function (sock: net.Socket): Q.Promise<net.Socket> {
            ...
            let cmd: string = RunOnDevice.makeGdbCommand('Hc0');
            initState++;
            sock.write(cmd);
            setTimeout(function (): void {
            ...
            let cmd: string = RunOnDevice.makeGdbCommand('c');
            initState++;
            sock.write(cmd);
            setTimeout(function (): void {
    ...
    private static mountDeveloperImage(): Q.Promise<any> {
        return RunOnDevice.getDiskImage()
...
process.on('exit', RunOnDevice.cleanup);
