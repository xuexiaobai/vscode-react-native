...
import {ProcessExecutionRecording, ProcessExecutionSimulator} from "./processExecutionSimulator";
...
const sampleRNProjectPath = path.join(resourcesPath, "myReactNative022Project");
...
export class ReactNative022 implements IReactNative {
    ...
    private simulator: ProcessExecutionSimulator = new ProcessExecutionSimulator({
        beforeStart: () => this.readAndroidPackageName(), // 1. We read the package.json to verify this is a RN project
        outputBased: [
            {
                pattern: /:app:assembleDebug/,
                action: () => this.createAPK() // 2. We compile the application. TODO: Execute this after the UP-TO-DATE
            },
            {
                pattern: /Installed on [0-9]+ devices*\./,
                action: () => this.installAppInAllDevices() //3. We install it on all available devices. TODO: Execute this for each individual device
            },
        ],
    ...
    public loadRecording(recording: ProcessExecutionRecording): void {
